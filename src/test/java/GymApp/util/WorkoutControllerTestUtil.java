...
public class WorkoutControllerTestUtil {
    ...
    static public ResponseEntity<WorkoutDto> attemptWorkoutCreation(WorkoutDto workoutDto, String token, int port, RestTemplate restTemplate) {

        // set up the request
        // As port number as it's generated randomly.
        String baseUrl = GeneralUtil.getBaseUrl(port);
        ...
        // send the request
        try {
            return restTemplate.exchange(baseUrl + "/workouts", HttpMethod.POST, request, WorkoutDto.class);
    ...
    static public ResponseEntity<WorkoutDto> attemptGetWorkoutById(long id, String token, int port, RestTemplate restTemplate) {
        ...
        // send the request
        try {
            return restTemplate.exchange(baseUrl + "/workouts/" + id, HttpMethod.GET, request, WorkoutDto.class);
        } catch (HttpClientErrorException e) {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
    }
    ...
    static public void shouldGetAllWorkouts(List<Workout> expectedWorkouts, String token, int port, RestTemplate restTemplate) {
        ...
        // convert workout entity to workoutDto.
        // why ? as we will compare it against workoutDto returned as response body.
        List<WorkoutDto> expectedWorkoutDto = expectedWorkouts.stream().map(WorkoutMapper::workoutEntityToWorkoutDto).toList();
    ...
    static public ResponseEntity<List<WorkoutDto>> attemptGetAllWorkouts(String token, int port, RestTemplate restTemplate) {
        ...
        ParameterizedTypeReference<List<WorkoutDto>> responseType = new ParameterizedTypeReference<List<WorkoutDto>>() {
        };
        // send the request
        try {
            return restTemplate.exchange(baseUrl + "/workouts", HttpMethod.GET, request, responseType);
        } catch (HttpClientErrorException e) {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
    }
    ...
    static public ResponseEntity<WorkoutDto> attemptUpdateWorkoutById(String token, int port, WorkoutDto workoutDto, RestTemplate restTemplate) {
        ...
        // send the request
        try {
            return restTemplate.exchange(baseUrl + "/workouts/" + workoutDto.id(), HttpMethod.PUT, request, WorkoutDto.class);
        } catch (HttpClientErrorException e) {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
    ...
    public static void shouldUpdateWorkoutById(String token, Workout workout, int port, RestTemplate restTemplate) {
        ...
        ResponseEntity<WorkoutDto> response = WorkoutControllerTestUtil.attemptUpdateWorkoutById(token, port, expectedWorkoutDto, restTemplate);
    ...
    public static void shouldNotUpdateWorkoutById(String token, Workout workout, int port, RestTemplate restTemplate) {
        ...
        ResponseEntity<WorkoutDto> response = attemptUpdateWorkoutById(token, port, expectedWorkoutDto, restTemplate);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);

    }

    static public ResponseEntity<WorkoutDto> attemptDeleteWorkoutById(String token, Workout workout, int port, RestTemplate restTemplate) {
        // set up the request
        // As port number as it's generated randomly.
        String baseUrl = GeneralUtil.getBaseUrl(port);
        ...
        // send the request
        try {
            return restTemplate.exchange(baseUrl + "/workouts/" + workout.getId(), HttpMethod.DELETE, request, WorkoutDto.class);
        } catch (HttpClientErrorException e) {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
